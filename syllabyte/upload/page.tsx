    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-6 py-12">
        ...
        <header className="flex items-center justify-between mb-16 animate-fade-up">
          ...
          <span className="text-[10px] font-mono text-slate-400 bg-white/60 border border-slate-200 rounded-full px-3 py-1.5">
            Professor Portal
          </span>
        </header>
        ...
        <div className="mb-8 animate-fade-up" style={{ animationDelay: "0.3s" }}>
          ...
          >
            <div className="w-12 h-12 rounded-xl gradient-bg flex items-center justify-center mx-auto mb-4 shadow-sm">
              <span className="text-xl text-white">{dragOver ? "📂" : "📁"}</span>
            </div>
              ...
              {files.map((file, i) => {
                ...
                return (
                  <div key={`${file.name}-${i}`} className="flex items-center gap-3 px-5 py-3.5">
                    <span className="text-lg flex-shrink-0">{getExtIcon(file.name)}</span>
                    ...
                    {result ? (
                      ...
                      <button
                        ...
                        className="text-slate-300 hover:text-red-400 transition-colors cursor-pointer text-lg"
                      >
                        ×
                      </button>
                    )}
        ...
        <div className="animate-fade-up" style={{ animationDelay: "0.4s" }}>
          <button
            ...
            className="w-full gradient-bg text-white rounded-xl py-4 text-sm font-semibold hover:opacity-90 disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer shadow-lg shadow-teal-500/20"
          >
            {uploading
              ? "Uploading to Raspberry Pi..."
              : `Upload ${files.length} file${files.length !== 1 ? "s" : ""} to ${courseId.trim().toUpperCase() || "..."}`}
          </button>
        ...
        {results.length > 0 && results.every((r) => r.status === "success") && (
          <div className="mt-8 border border-teal-200 bg-teal-50/50 rounded-2xl p-6 text-center animate-fade-up">
            ...
            <Link
              ...
              className="inline-block text-sm font-medium text-white gradient-bg rounded-full px-6 py-2.5 shadow-sm hover:opacity-90 transition-opacity"
            >
              Try the assistant →
            </Link>
          </div>
